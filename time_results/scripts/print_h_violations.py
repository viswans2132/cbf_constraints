...
def main():
    ...
    for i in range(no_agents):
        ...
        if np.isnan(pct):
            # pct = 0.0
            continue
        landing_values.append(pct)
        if pct > 1.0:
            print(f"  UAV {i+1}: {pct:.2f}%")
    print(f"  Average: {np.mean(landing_values):.2f}%\n")

    # 2) UAV boundary constraints
    if "boundary" in drone_logs[0]:
        ...
        for c in range(5):
            ...
            print(f"  Average {boundary_names[c]}: {np.mean(values):.2f}%")
        print()

    # 3) UAV-UGV air constraints
    air_h = {
    ...
    for (i, k), series in air_h.items():
        ...
        if np.isnan(pct):
            continue
            pct = 0.0
        air_values.append(pct)
        if pct > 1.0:
            print(f"  UAV {i+1} wrt UGV {k+1}: {pct:.2f}%")
    if air_values:
        print(f"  Average: {np.mean(air_values):.2f}%")
    print()

    # 4) UAV-UAV pair constraints
    drone_pair_h = {
    ...
    if pair_values:
        print(f"  Average: {np.mean(pair_values):.2f}%")
    print()

    print("========== UGV CONSTRAINT VIOLATION PERCENTAGES ==========\n")

    # 5) UGV boundary constraints
