...
cat <<EOL >> $launch_file
  ...
  <node pkg="cbf_constraints" name="centralized_controller" type="master_control_node.py" output="screen">
    <param name="no_of_agents" value="$pairs" />
  </node>
  <node pkg="cbf_constraints" name="control_relay" type="control_relay.py" output="screen">
    <param name="no_of_agents" value="$pairs" />
  </node>
EOL
