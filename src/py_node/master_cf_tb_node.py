...
class CentralController:
    ...
    def getUgvPosVelMsg(self, msg, i, offset, time):
        ...
        if i == 1:
            ...
            msg.position = [-1.0*np.cos(offset + time/8) + self.offsetW[0], -0.3*np.sin(offset + time/8) + self.offsetW[1]]
            msg.velocity = [1.0*np.sin(offset + time/8)/8, -0.3*np.cos(offset + time/10)/8]
            # # if (int(time/30))%2 == 0:
            # if flag:    
